...
def _resolve_org_id(db: Session, system_id: str) -> str:
    """Look up org_id for a system. One DB hit per update_entity_centroids call."""
...
def update_entity_centroids(system_id: str, new_claim_ids: List[str]) -> List[str]:
    ...
    organizations FK. New EntityBeliefState rows carry org_id.
    """
    ...
    try:
        ...
        for claim in claims:
            ...
            if not belief_state:
                ...
                # Defensive: backfill org_id if the row pre-dates Sprint 1.3
                if belief_state.org_id is None:
                    belief_state.org_id = org_id
        ...
        db.commit()
        logger.info(f"OBG Updater: Shifted centroids & variance for {len(updated_entities)} entities (org={org_id}).")
